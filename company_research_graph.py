...
def route_after_eval(state: CompanyResearchState) -> str:
    ...
    if next_action == "report" or force_report_generation:
        if force_report_generation and next_action != "report":
            logger.info(
                "[graph/route_after_eval] force_report_generation=True overriding next_action=%s",
                next_action,
            )
        else:
